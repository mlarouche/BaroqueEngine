...
inline Baroque::Formatting::internal::null<> strerror_r(int, char *, ...) {
  return Baroque::Formatting::internal::null<>();
}
inline Baroque::Formatting::internal::null<> strerror_s(char *, std::size_t, ...) {
  return Baroque::Formatting::internal::null<>();
}
...
typedef void (*FormatFunc)(internal::buffer &, int, Baroque::StringView);
...
FMT_FUNC size_t internal::count_code_points(Baroque::StringView s) {
  auto* data = s.Data();
...
FMT_FUNC void system_error::init(
    int err_code, Baroque::StringView format_str, format_args args) {
  error_code_ = err_code;
  MemoryBuffer buffer;
  format_system_error(buffer, err_code, VFormat(format_str, args));
  std::runtime_error &base = *this;
  base = std::runtime_error(ToString(buffer).Data());
}
...
FMT_FUNC void format_system_error(
	internal::buffer &out, int error_code, Baroque::StringView message) FMT_NOEXCEPT {
  FMT_TRY {
    MemoryBuffer buf;
...
FMT_FUNC void report_system_error(
    int error_code, Baroque::StringView message) FMT_NOEXCEPT {
  report_error(format_system_error, error_code, message);
}

FMT_FUNC void VPrint(std::FILE *f, Baroque::StringView format_str, format_args args) {
  MemoryBuffer buffer;
  internal::VFormatTo(buffer, format_str,
                       basic_format_args<buffer_context<char>::type>(args));
  std::fwrite(buffer.data(), 1, buffer.size(), f);
}

FMT_FUNC void VPrint(Baroque::StringView format_str, format_args args) {
